...
?>
   ...
   <?php
     ...
     $allOffense = getdb("*","offenses","WHERE","place_violation = '$station'");
      if(!empty($allOffense)){
    ?>
        <h2 class="text-center mb-3">All Offenses</h2>
        <h2 class='mb-3'>Station :  <?php echo $trafficPo[0]['nameSta'] ?></h2>
...
<?php 
    ...
    if($do == "view"){
       ...
       ?>
        ...
        <div class="container allof">
            ...
            <?php
              if($items[0]['paidStatut']==1 || $items[0]['paidStatut']==2 || $items[0]['paidStatut']==3 ){
    ...
    if($do == 'update'){
      ...
      $slct = $_POST['slct'];
       ...
       $stmt = $con->prepare("UPDATE `offenses` SET `paidStatut` = 1 , `PaidBy` = 1 , `id_pay` = '$slct' WHERE `offenses`.`id` = '$id' ");
       $stmt->execute();
       $count = $stmt->rowCount();
